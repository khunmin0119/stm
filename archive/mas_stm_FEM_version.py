"""
MAS-based STM Generator — FEM Version
=======================================
3-Agent pipeline with stiffness matrix-based truss analysis for scoring.
...
Score function:
  Uses 2D direct stiffness method to solve truss member forces.
  Score = Sum(Ti x Li) for tie members (tension).
  This is proportional to total reinforcement needed — lower = better.
  Falls back to geometric score if truss analysis fails.

Image outputs:
  01_C{n}_nodes_A{a}_{PASS|FAIL}.png
  02_C{n}_conn_A{a}_{PASS|FAIL}.png
...
# ===========================================================
# 3-B. Truss Analysis (Direct Stiffness Method)
# ===========================================================
...
def solve_truss(nodes, connections, supports, loads_dict):
    ...
    Args:
        ...
        supports: dict {"A": "pin", "D": "roller"}
        loads_dict: dict {"F": [0, -2000000], ...} — forces in Newtons

    Returns:
        dict with 'member_forces', 'displacements', 'reactions', 'success', 'error'
        member_forces: positive = tension (tie), negative = compression (strut)
    """
    ...
    # Uniform EA (for relative comparison, absolute value cancels out in ranking)
    EA = 200000.0 * 1000.0  # E=200000 MPa, A=1000 mm^2
...
def score_stm(stm_data, beam_length, beam_height, loads, support_positions):
    ...
    Primary score: Sum(Ti x Li) for tie members — proportional to total reinforcement.
    Fallback: geometric total member length if truss analysis fails.

    Args:
        loads: [(x, Fy_kN), ...] — load forces in kN (will be converted to N internally)

    Returns:
        float: normalized score (lower = better)
    """
    nodes = stm_data['nodes']
    connections = stm_data['connections']
    supports = stm_data['supports']

    # Build loads_dict: find top-chord node at each load x, convert kN → N
    loads_dict = {}
...
def score_stm_detailed(stm_data, beam_length, beam_height, loads, support_positions):
    """
    Full truss analysis with detailed results for final output.
    Returns dict with score, member forces, reactions, etc.
    """
    nodes = stm_data['nodes']
    connections = stm_data['connections']
    supports = stm_data['supports']
...
def print_truss_analysis(detail):
    ...
    if not detail.get('analysis_success'):
        ...
        print(f"  [Fallback] Geometric score = {detail.get('score', '?'):.2f}")
        return
...
def plot_candidates_comparison(candidates_info, beam_length, beam_height,
    ...
    for idx, (stm, sc, cid) in enumerate(candidates_info):
        ...
        ax.set_title(
            f'Candidate {cid}  score={sc:.4f}  {badge}\n'
...
def plot_optimizer_comparison(before_stm, after_stm, before_score, after_score,
    ...
    for ax, stm, label, sc in [(ax1, before_stm, 'Before Optimization', before_score),
        ...
        ax.set_title(f'{label}\nscore={sc:.4f} | {len(nodes)} nodes, {len(connections)} members',
                     fontsize=11, fontweight='bold', pad=10)
...
class MASSTMGenerator:
    ...
    def __init__(self,
        ...
        self.topology_model = topology_model      # Node Placer (JSON reasoning)
        self.coder_model = coder_model            # Connection Planner + Optimizer (code generation)
        self.reviewer_model = reviewer_model      # Engineering Review (assessment)
    ...
    def _plan_connections(self, nodes_data, L, H,
        ...
        response = self.client.chat(
            model=self.coder_model, system=CONNECTION_PLANNER_PROMPT,
            user=user_prompt, temperature=0.3
        )
            ...
            if candidate is not None:
                ...
                if verbose:
                    print(f"  → Candidate {i+1} (x1={x1_values[i]}): "
                          ...
                          f"{len(candidate['connections'])} members, score={sc:.4f}")
            else:
                if verbose:
    ...
    def _retry_with_review_feedback(self, stm_data, review, L, H, loads, support_positions, verbose=True):
        ...
        for attempt in range(1, 3):  # 2 attempts to fix
            ...
            if val_result['valid']:
                ...
                if verbose:
                    print(f"  │   [Validator] PASS ✓ (score: {old_score:.4f} → {new_score:.4f})")
                # Only accept if score improved (lower = better)
                if new_score < old_score:
                    return candidate
                else:
                    if verbose:
                        print(f"  │   Score not improved, keeping original")
                    continue
            else:
                if verbose:
                    print(f"  │   [Validator] FAIL")
    ...
    def _optimize(self, stm_data, L, H, loads, support_positions, verbose=True):
        if verbose:
            print(f"\n{'─' * 55}")
            print(f"  PHASE 3: Optimization ({self.coder_model})")
            print(f"{'─' * 55}")
        ...
        for attempt in range(1, 4):  # 3 attempts
            ...
            t0 = time.time()
            response = self.client.chat(
                model=self.coder_model, system=OPTIMIZER_AGENT_PROMPT,
                user=user_prompt,
                temperature=0.3,
                force_json=False
            )
            ...
            if after_score < before_score:
                ...
                opt_feedback = (
                    f"Your code produced a valid STM but score={after_score:.4f} "
    ...
    def generate(self, beam_length, beam_height, beam_width,
        ...
        if verbose:
            print("=" * 65)
            print("  MAS-based STM Generation — FEM Version")
            print("=" * 65)
        ...
        plot_stm(
            ...
            title=f'★ Best Candidate (score={best_score:.4f})',
            status='PASS'
        )
        ...
        if review.get('assessment') == 'QUESTIONABLE' and review.get('issues'):
            ...
            if improved_stm:
                ...
                best_score = score_stm(best_stm, L, H, loads, support_positions)
                if verbose:
                    print(f"  [Reviewer Loop] Improved STM accepted (score={best_score:.4f})")
        ...
        plot_stm(
            ...
            title=f'FINAL STM (score={final_score:.4f}, time={elapsed:.0f}s)',
            status='PASS'
        )
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 65)
    print("  MAS STM Generator — FEM Version")
    print("=" * 65)
